...
_ERROR_MESSAGE_PROMPT_LEAK_KEYS = ("input", "messages")
...
def _scan_balanced_value_end(text: str, start: int) -> int:
    ...
    while i < n:
        c = text[i]
        if c in ("'", '"'):
            end = _scan_balanced_value_end(text, i)
            if end == -1:
                return -1
            i = end
            continue
...
def _redact_prompt_leaks_in_error_string(text: str) -> str:
    ...
    Locates ``"<key>": <value>`` pairs where ``key`` is ``input`` or
    ``messages`` and replaces the value with the LiteLLM redaction marker.
...
def _sanitize_error_information_for_spend_logs(
    ...
    - When ``store_prompts_in_spend_logs`` is False: additionally redact
      ``'input'`` / ``'messages'`` values inside both ``error_message`` *and*
